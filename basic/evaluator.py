...
class MultiGPUF1Evaluator(F1Evaluator):
    def __init__(self, config, models, tensor_dict=None):
        ...
        with tf.name_scope("eval_concat"):
            ...
            self.yp = tf.concat([padded_reshape(model.yp, [N, M, JX]) for model in models], axis=0)
            self.yp2 = tf.concat([padded_reshape(model.yp2, [N, M, JX]) for model in models], axis=0)
